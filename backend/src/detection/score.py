...
def generate_spread_heatmap(
    image,
    detection_results,
    severity_map,
    default_s_i,
    weighting=DEFAULT_HEATMAP_WEIGHTING,
    alpha=DEFAULT_HEATMAP_ALPHA,
    ...
    print("TRYING TO GENERATE HEATMAP")
    print(image.shape)
    if not isinstance(image, np.ndarray) or image.ndim != 3:
        return image, np.zeros(
            image.shape[:2] if isinstance(image, np.ndarray) else (100, 100),
            dtype=np.float32,
        )
    if not detection_results or len(detection_results) == 0:
        return image, np.zeros(image.shape[:2], dtype=np.float32)
    try:
    ...
    if num_detections > 0:
        ...
        for box in boxes:
            try:
                ...
                continue
        if points_data:
            for y, x, weight in points_data:
                heatmap_raw[y, x] += weight
            if spread_sigma > 0:
                heatmap_spread = gaussian_filter(heatmap_raw, sigma=spread_sigma)
            else:
                heatmap_spread = heatmap_raw
            if (
                secondary_blur_ksize
                and secondary_blur_ksize > 1
                and secondary_blur_ksize % 2 == 1
            ):
                heatmap_blurred = cv2.GaussianBlur(
                    heatmap_spread, (secondary_blur_ksize, secondary_blur_ksize), 0
                )
            else:
                heatmap_blurred = heatmap_spread
            ...
            if max_val > 1e-6:
                ...
                heatmap_norm = np.zeros(heatmap_blurred.shape, dtype=cv2.CV_8U)
            ...
            heatmap_overlay = cv2.addWeighted(
                heatmap_color, alpha, image_uint8, 1 - alpha, 0
            )
            return heatmap_overlay, heatmap_norm
        else:
            return image, heatmap_raw
    else:
        return image, heatmap_raw
...
def calculate_acneai_score(detection_results, image_shape, severity_map, default_s_i):
    ...
    score_range = (0, 100)
    if not detection_results or len(detection_results) == 0:
        return score_range[0], 0.0, 0.0, 0
    ...
    if N_total_boxes == 0 or A <= 0:
        return score_range[0], 0.0, 0.0, 0
    sum_term = 0.0
    total_lesion_area = 0.0
    sum_severity_points = 0.0
    valid_detections = 0
    for box in boxes:
        try:
            x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
            ...
            if w <= 0 or h <= 0:
                continue
            a_i = float(w * h)
            total_lesion_area += a_i
            class_id = int(box.cls[0])
            class_name = names.get(class_id, f"class_{class_id}")
            s_i = severity_map.get(class_name, default_s_i)
            sum_severity_points += s_i
            sum_term += (s_i * a_i) / A
            valid_detections += 1
        except (AttributeError, IndexError, TypeError) as e:
            continue
    ...
    if N == 0:
        return score_range[0], 0.0, 0.0, 0
    try:
        inner_term = 20.0 * sum_term
        score_S = (200.0 / math.pi) * math.atan(inner_term)
    except Exception as e:
        print(f"Error during score math calc: {e}")
        score_S = score_range[0]
...
def analyze_skin_image(
    ...
    results = {
        ...
        "severity_score": 0.0,
        "percentage_area": 0.0,  # Initialize with defaults
        "average_intensity": 0.0,
        "lesion_count": 0,
        "original_image_bgr": None,
        "heatmap_overlay_bgr": None,
        "detections": [],
        "model_classes": {},
    }
    ...
    try:
        ...
        if not isinstance(results["model_classes"], dict):
        ...
        predict_results = model.predict(
            source=image_path, conf=conf_threshold, save=False
        )
        ...
        if (
            predict_results
            and len(predict_results) > 0
            and hasattr(predict_results[0], "boxes")
            and len(predict_results[0].boxes) > 0
        ):
        ...
        results["message"] = (
            "Analysis completed successfully using AcneAI score formula."
        )
...
if __name__ == "__main__":
    ...
    analysis_results = analyze_skin_image(
        model_path=example_model_path, heatmap_sigma=DEFAULT_GAUSSIAN_SPREAD_SIGMA
    )
    ...
    if analysis_results and analysis_results["success"]:
        ...
        for det in analysis_results["detections"]:
            print(f"  - Class: {det['class_name']}, Conf: {det['confidence']:.3f}")
        ...
        if example_output_path and analysis_results["heatmap_overlay_bgr"] is not None:
            try:
                ...
                success = cv2.imwrite(
                    example_output_path, analysis_results["heatmap_overlay_bgr"]
                )
                if success:
                    print(f"\nOutput image saved to: {example_output_path}")
                    output_filename = os.path.basename(example_output_path)
                else:
                    print(
        ...
        if (
            display_results
            and analysis_results["original_image_bgr"] is not None
            and analysis_results["heatmap_overlay_bgr"] is not None
        ):
            print("\n--- Displaying Results ---")
            image_rgb = cv2.cvtColor(
                analysis_results["original_image_bgr"], cv2.COLOR_BGR2RGB
            )
            heatmap_overlay_rgb = cv2.cvtColor(
                analysis_results["heatmap_overlay_bgr"], cv2.COLOR_BGR2RGB
            )
            fig, axes = plt.subplots(1, 2, figsize=(16, 8))
            fig.suptitle(
                f"Analysis for: {os.path.basename(example_image_path)}", fontsize=16
            )
